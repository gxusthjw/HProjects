...
class LogisticTest extends UnitSpec {
  ...
  "the derivative of Logistic" should "equal the differential of Logistic" in {
    ...
    assert(logistic.derivative(0) - logistic.differential(0) <= NumericalConstants.NUMERICAL_PRECISION)
    assert(logistic.derivative(100) - logistic.differential(100) <= NumericalConstants.NUMERICAL_PRECISION)
    assert(logistic.derivative(-100) - logistic.differential(-100) <= NumericalConstants.NUMERICAL_PRECISION)
    assert(logistic.derivative(200) - logistic.differential(200) <= NumericalConstants.NUMERICAL_PRECISION)
    assert(logistic.derivative(-200) - logistic.differential(-200) <= NumericalConstants.NUMERICAL_PRECISION)
    assert(logistic.derivative(10) - logistic.differential(10) <= NumericalConstants.NUMERICAL_PRECISION)
    assert(logistic.derivative(5) - logistic.differential(5) <= NumericalConstants.NUMERICAL_PRECISION)
  }
  ...
  "the integrate of Logistic" should "equal to integralSimson of Logistic" in {
    ...
    assert(logistic.integrateIterativeLegendreGauss(0, 100) - logistic.integrate(0, 100) <= 1e-7)
  }
